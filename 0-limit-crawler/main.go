...
// const rateLimit = time.Second/1 //1 call per second

func Crawl(url string, depth int, wg *sync.WaitGroup, rateLimit *time.Ticker) {
	defer wg.Done()
	...
	for _, u := range urls {
		...
		// called concurrently
		go Crawl(u, depth-1, wg, rateLimit)
	}
}
...
func main() {
	...
	Crawl("http://golang.org/", 4, &wg, rateLimit)
	wg.Wait()
}
